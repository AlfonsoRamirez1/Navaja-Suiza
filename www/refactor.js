...
// Tokens Material 3 comunes para inyectar en CSS
...
function processFolder(dir) {
    ...
    if (fs.existsSync(indexPath) && fs.existsSync(stylePath)) {
        console.log('Procesando: ' + path.basename(dir));
        
        // --- 1. PROCESAR HTML ---
        const html = fs.readFileSync(indexPath, 'utf-8');
        const $ = cheerio.load(html);

        // Actualizar HTML y head
        ...
        // Extraer título
        const h1Text = $('header h1').text().trim() || path.basename(dir);
        
        // Reemplazar header por AppBar
        $('header').replaceWith('    <header class="app-bar">\\n' +
        ...
        '            <button class="icon-button tooltip-container" id="theme-toggle" aria-label="Cambiar tema">\\n' +
        ...
        // Cambiar container a content
        const container = $('.container');
        if (container.length) {
            container.removeClass('container').addClass('content');
            // Mover AppBar fuera del content
            $('body').prepend($('.app-bar'));
        }
        ...
        // Remover variables viejas y selectores base conflictivos
        ...
        css = css.replace(/\.icon-container\s*{[\s\S]*?}/, '');
        css = css.replace(/header h1\s*{[\s\S]*?}/, '');
        css = css.replace(/header p\s*{[\s\S]*?}/, '');
        ...
        css = css.replace(/rgba\(15, 23, 42, 0\.4\)/g, 'var(--md-sys-color-surface-container)');
        css = css.replace(/rgba\(30, 41, 59, 0\.6\)/g, 'var(--md-sys-color-surface)');
        css = css.replace(/rgba\(30, 41, 59, 0\.7\)/g, 'var(--md-sys-color-surface)');
...
console.log('¡Refactorización completa!');
